            ...
            <div class="relative overflow-x-auto bg-white border rounded-lg">
                <div class="flex w-full">
                    <div class="w-full rounded-lg border-b-3">
                        ...
                        <form action="/jadwal/seminarproposal/store" method="post" class="" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="p-6 lg:p-6 ">
                                <div class="grid grid-cols-1 mb-5 lg:grid-cols-2 lg:mx-10 gap-7 ">
                                    <div class="mb-3">
                                        ...
                                        <select id="id_proposal" name="id_proposal" onchange="redirectToById(this)"
                                            ...
                                            @foreach ($listsempro as $sempro)
                                                <option value="{{ $sempro->id }}" {{ $sempro->id == $semprobyid->id ? 'selected' : '' }}>{{ $sempro->nama_mahasiswa }} /
                                                    {{ $sempro->nim }}</option>
                                            @endforeach
                                        </select>
                                        <script>
                                    ...
                                    <div class="mb-3">
                                        <input type="text" name="ketua_tim" hidden value="{{ $semprobyid->pembimbing1 }}">
                                        <label for="pembimbing1"
                                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                            Pembimbing Utama (Ketua Tim)</label>
                                        <input type="text" id="pembimbing1" disabled name="pembimbing1" value="{{ $semprobyid->pembimbing1 }}"
                                            placeholder="{{ $semprobyid->pembimbing1 }}"
                                    ...
                                    <div class="mb-3">
                                        <input type="text" name="anggota" hidden value="{{ $semprobyid->pembimbing2 }}">
                                        <label for="pembimbing2"
                                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                            Pembimbing Kedua (Anggota)</label>
                                        <input type="text" id="pembimbing2" disabled name="pembimbing2" value="{{  $semprobyid->pembimbing2 }}"
                                            placeholder="{{ $semprobyid->pembimbing2 }}"
